...
public class CalculadoraCientificaImpl extends UnicastRemoteObject implements CalculadoraCientifica {
    private boolean hasError; // Estado de error
    private BigDecimal resultado; // Almacena el resultado de las operaciones

    // Constructor
    public CalculadoraCientificaImpl() throws RemoteException {
        super();
        this.hasError = false;
   ...
   /* // Suma
    ...
    public BigDecimal porcentaje(BigDecimal parte, BigDecimal total) throws RemoteException {
        if (total.compareTo(BigDecimal.ZERO) == 0) {
            manejarError(); // Marca un error si el total es cero
            throw new ArithmeticException("El total no puede ser cero");
        }
        // Calcula el 1% del valor ingresado
        BigDecimal resultado = parte.divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP); 
        return resultado;
    }
*/
   ...
   @Override
   public double calcularSeno(double valor, boolean isDegMode) throws RemoteException {
       if (isDegMode) {
           // Convertir grados a radianes si está en modo grados
